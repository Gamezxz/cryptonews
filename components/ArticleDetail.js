...
export default function ArticleDetail({ article, relatedArticles }) {
  ...
  const tags = articleData.categories?.length > 0
    ? articleData.categories
    : [articleData.category];

  const hasSummary = articleData.aiSummary || articleData.aiSummaryThai;
  const hasFullContent = articleData.fullContent && articleData.fullContent.length > 100;

  async function handleScrape() {
    setScraping(true);
    try {
      const res = await fetch(`http://localhost:13002/api/news/${articleData._id}/scrape`, {
        method: 'POST'
      });
      const data = await res.json();
      if (data.success && data.data) {
        setArticleData(prev => ({ ...prev, ...data.data }));
      }
    } catch (err) {
    ...
    <div className="article-detail">
      ...
      <header className="article-header">
        <div className="article-badges">
          ...
          {articleData.sentiment && (
            <span className={`sentiment-badge sentiment-${articleData.sentiment}`}>
              {articleData.sentiment === 'bullish' ? '↑ Bullish' :
               articleData.sentiment === 'bearish' ? '↓ Bearish' : '— Neutral'}
            </span>
          )}
          {articleData.wordCount > 0 && (
            <span className="reading-time-badge">{readingTime(articleData.wordCount)}</span>
          )}
        </div>
        ...
        <div className="article-meta-row">
          <time dateTime={articleData.pubDate}>{timeAgo(articleData.pubDate)}</time>
      ...
      {hasSummary ? (
        ...
        <section className="summary-box summary-box-empty">
          ...
          {!hasFullContent && !scraping && (
            <button onClick={handleScrape} className="scrape-btn">
              Load Full Article &amp; Generate Summary
            </button>
          )}
          {scraping && (
            <div className="scraping-status">
              <span className="scraping-spinner"></span>
              Scraping &amp; summarizing...
            </div>
          )}
          {hasFullContent && !hasSummary && (
            <p className="summary-pending">Content scraped. Summary generation pending.</p>
          )}
        </section>
      )}
        ...
        <section className="related-articles">
          ...
          <div className="related-grid">
            {relatedArticles.map(rel => (
              <Link href={`/news/${rel._id}/`} key={rel._id} className="related-card">
